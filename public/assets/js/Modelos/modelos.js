    ...
    Alpine.data('multipleTable', () => ({
            ...
            fetch('/api/modelo')
                ...
                .then((data) => {
                    ...
                    this.datatable1 = new simpleDatatables.DataTable('#myTable1', {
                        data: {
                            headings: ['Nombre', 'Marca', 'Categoría', 'Estado', 'Acción'],
        ...
        formatDataForTable(data) {
            return data.map((modelo) => [
                `<div style="text-align: center;">${modelo.nombre}</div>`, // Columna: Nombre
                `<div style="text-align: center;">${modelo.marca || 'Sin Marca'}</div>`, // Columna: Marca
                `<div style="text-align: center;">${modelo.categoria || 'Sin Categoría'}</div>`, // Columna: Categoría
                ...
                `<div style="text-align: center;" class="flex justify-center items-center">
                    <a href="/modelos/${modelo.idModelo}/edit" class="ltr:mr-2 rtl:ml-2" x-tooltip="Editar">
                    ...
                    <button type="button" x-tooltip="Delete" @click="deleteModel(${modelo.idModelo})">
        ...
        deleteModel(idModelo) {
            new window.Swal({
                ...
                if (result.value) {
                    console.log(`Iniciando eliminación del modelo con ID: ${idModelo}`);
