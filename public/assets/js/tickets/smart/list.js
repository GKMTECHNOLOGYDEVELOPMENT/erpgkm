    ...
    Alpine.data('multipleTable', () => ({
      ...
      formatDataForTable(data) {
        return data.map(orden => {
            ...
            const wrap = (text, style = '') =>
                `<div style="word-wrap: break-word; white-space: normal; text-align: center; ${style}">${text}</div>`;
    
            // Aquí se obtiene el color del estadoflujo
            const estadoColor = orden.estadoflujo && orden.estadoflujo.color ? orden.estadoflujo.color : '';
    
            // Si el color está presente, se aplica como el fondo de la fila
            const rowStyle = estadoColor ? `background-color: ${estadoColor};` : '';
    
            console.log('Categoria:', orden.modelo ? orden.modelo.categoria : 'N/A');
            console.log('Estado Flujo:', orden.estadoflujo); // Verifica los datos
    
            return [
                // Primera columna (Icono de editar)
                `<div style="text-align: center; ${rowStyle}" class="flex justify-center items-center">
                ...
                wrap(orden.numero_ticket || 'N/A', rowStyle),
    
                // Tercera columna (Fecha de creación)
                wrap(fechaTicket, rowStyle),
    
                // Cuarta columna (Fecha de visita)
                wrap(fechaVisita, rowStyle),
    
                // Quinta columna (Categoría del modelo)
                wrap(orden.modelo && orden.modelo.categoria ? orden.modelo.categoria.nombre : 'N/A', rowStyle),
    
                // Sexta columna (Descripción del cliente general)
                wrap(orden.clientegeneral ? orden.clientegeneral.descripcion : 'N/A', rowStyle),
    
                // Séptima columna (Nombre del modelo)
                wrap(orden.modelo ? orden.modelo.nombre : 'N/A', rowStyle),
    
                // Octava columna (Número de serie)
                wrap(orden.serie ? orden.serie : 'N/A', rowStyle),
    
                // Novena columna (Nombre del cliente)
                wrap(orden.cliente ? orden.cliente.nombre : 'N/A', rowStyle),
    
                // Décima columna (Dirección)
                wrap(orden.direccion || 'N/A', rowStyle),
    
                // Última columna (Botón de detalles)
                `<div class="flex justify-center items-center" style="${rowStyle}">
