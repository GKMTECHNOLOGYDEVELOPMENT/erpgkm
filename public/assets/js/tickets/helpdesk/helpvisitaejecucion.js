...
fetch(`/api/obtenerVisitas/${ticketId}`)
    ...
    .then((data) => {
        ...
        if (data && data.length > 0) {
            data.forEach((visita) => {
                ...
                // 📌 Botón para ocultar/mostrar la card (SOLO SI TIENE PERMISO)
                if (window.permisosHelpDeskVisitas.puedeOcultarMostrar) {
                    const toggleCardButton = document.createElement('button');
                    toggleCardButton.className =
                        'px-4 py-2 mb-2 bg-gray-300 text-gray-800 rounded-lg shadow hover:bg-gray-400 transition font-semibold w-full flex justify-between items-center';
  ...
  `;
                ...
                if (window.permisosHelpDeskVisitas.puedeSeleccionarVisita) {
                ...
                const viewImageButtonHtml = window.permisosHelpDeskVisitas.puedeVerImagenVisita ? 
          ...
          `;
                ...
                if (window.permisosHelpDeskVisitas.puedeVerDetalles) {
...
`;
                    ...
                    detailsButton.addEventListener('click', function () {
                            ...
                            fetch(`/api/ticketapoyo/${visita.idVisitas}/${visita.idTickets}`)
                                ...
                                .then((tecnicosApoyo) => {
                                    ...
                                    tecnicosApoyo.forEach((tecnico) => {
                                        ...
                                        if (window.permisosHelpDeskVisitas.puedeEliminarTecnicoApoyo) {
                        ...
                        if (window.permisosHelpDeskVisitas.puedeAgregarTecnicoApoyo) {
                    ...
                    if (window.permisosHelpDeskVisitas.puedeActualizarVisita) {
                ...
                const iniciarDesplazamientoHtml = window.permisosHelpDeskVisitas.puedeIniciarDesplazamiento ? 
                    `<div class="flex justify-center mt-2">
                ...
                // Botón Ver Imagen Desplazamiento (SOLO SI TIENE PERMISO)
                const verImagenDesplazamientoHtml = window.permisosHelpDeskVisitas.puedeVerImagenDesplazamiento && visita.recojo === 1 ? 
                    `<button 
                ...
                fetch(`/api/verificarRegistroAnexo/${visita.idVisitas}`)
                    ...
                    .then((anexoData) => {
                        if (anexoData && anexoData.idVisitas) {
                            ...
                            // Botones con permisos para Inicio de Servicio
                            const subirFotoHtml = window.permisosHelpDeskVisitas.puedeSubirFoto ? 
                            ...
                            const siguienteHtml = window.permisosHelpDeskVisitas.puedeSiguiente ? 
                            ...
                            const verImagenInicioHtml = window.permisosHelpDeskVisitas.puedeVerImagenInicioServicio ? 
                                `<button 
                                class="flex items-center justify-center gap-2 
                                       px-2 py-1 sm:px-3 sm:py-1.5
                                       text-xs sm:text-sm md:text-base
                            ...
                            fetch(`/api/verificarFechaExistente/${visita.idVisitas}`)
                                ...
                                .then((data) => {
                                    if (data.existe) {
                                        ...
                                        const continuarHtml = window.permisosHelpDeskVisitas.puedeContinuar && visita.servicio !== 1 ? 
                                  ...
                                  <span class="text-xs sm:text-sm">Continuar Aquí</span>
                                        ...
                                        const verImagenFinalHtml = window.permisosHelpDeskVisitas.puedeVerImagenFinalServicio && visita.servicio === 1 ? 
                                            `<button class="flex items-center justify-center gap-2
                                          px-2 py-1 sm:px-3 sm:py-1.5
                                          text-xs sm:text-sm md:text-base
                                          font-semibold rounded-full shadow-md 
  ...
  ${visita.anexos_visitas.length > 0 && visita.anexos_visitas.find((anexo) => anexo.idTipovisita === 4) ? visita.anexos_visitas.find((anexo) => anexo.idTipovisita === 4).ubicacion : 'Ubicación no disponible'}
                      ...
                      `;
                                        ...
                                        // Agregar el evento de clic al botón "Continuar" (SOLO SI TIENE PERMISO)
                                        if (window.permisosHelpDeskVisitas.puedeContinuar && visita.servicio !== 1) {
                            ...
                            if (window.permisosHelpDeskVisitas.puedeSiguiente) {
                                const siguienteButton = document.getElementById(`siguiente-${visita.idVisitas}`);
                                if (siguienteButton) {
                                    siguienteButton.addEventListener('click', () => {
                                        ...
                                        // Botón Continuar (SOLO SI TIENE PERMISO)
                                        const continuarFinalHtml = window.permisosHelpDeskVisitas.puedeContinuar ? 
                                      ...
                                      <span class="text-xs sm:text-sm">Continuar Aqui si estoy </span>
                                        ...
                                        // Botón Ver Imagen Final (SOLO SI TIENE PERMISO)
                                        const verImagenFinalHtml = window.permisosHelpDeskVisitas.puedeVerImagenFinalServicio ? 
                                            `<button class="flex items-center justify-center gap-2 
                                    px-2 py-1 sm:px-3 sm:py-1.5
                                    text-xs sm:text-sm md:text-base
                                    font-semibold rounded-full shadow-md 
                  ...
                  `;
                                        ...
                                        // Agregar el evento de clic al botón "Finalizar" (SOLO SI TIENE PERMISO)
                                        if (window.permisosHelpDeskVisitas.puedeContinuar) {
                            ...
                            if (window.permisosHelpDeskVisitas.puedeSubirFoto) {
                                const uploadPhotoButton = document.getElementById(`uploadPhotoButton-${visita.idVisitas}`);
                                const fileInput = document.getElementById(`fileInput-${visita.idVisitas}`);

                                if (uploadPhotoButton && fileInput) {
                                                                ...
                                                                fetch('/api/subirFoto', {
                                                                    ...
                                                                    .then((data) => {
                                                                        if (data.success) {
                                                                            ...
                                                                            if (uploadPhotoButton) uploadPhotoButton.style.display = 'none';
                                                                            if (siguienteButton) siguienteButton.style.display = 'block'; // Habilitar el botón "Siguiente"
                                                                            // Agregar el evento de clic al botón "Continuar" después de subir la foto (SOLO SI TIENE PERMISO)
                                                                            if (window.permisosHelpDeskVisitas.puedeContinuar) {
                                                                                const continueButton = document.getElementById(`continueButton-${visita.idVisitas}`);
                                                                                if (continueButton) {
                                                                                    continueButton.addEventListener('click', () => {
                ...
                if (window.permisosHelpDeskVisitas.puedeIniciarDesplazamiento) {
                                                                ...
                                                                fetch('/api/guardarAnexoVisita', {
                                                                    ...
                                                                    .then((data) => {
                                                                        ...
                                                                        if (data.success) {
                                                                            ...
                                                                            // Si existe el registro, mostrar la tarjeta de "Inicio de Servicio"
                                                                            const inicioServicioCard = document.createElement('div');
                                                                            inicioServicioCard.className =
                                                                                'rounded-lg shadow-md p-4 w-full sm:max-w-md mx-auto bg-[#d9f2e6]';
                                                                    ...
                                                                    <span class="text-xs sm:text-sm">Siguiente</span>
                                                                            ...
                                                                            const verImagenHtml = window.permisosHelpDeskVisitas.puedeVerImagenInicioServicio ? 
                                                                                `<button class="bg-success text-white px-2 py-1 sm:px-3 sm:py-1.5 rounded-full shadow-md
  ...
  ${visita.anexos_visitas.length > 0 && visita.anexos_visitas.find((anexo) => anexo.idTipovisita === 3) ? visita.anexos_visitas.find((anexo) => anexo.idTipovisita === 3).ubicacion : 'Ubicación no disponible'}
                                                                            ...
                                                                            if (window.permisosHelpDeskVisitas.puedeSiguiente) {
                                                                                ...
                                                                                if (siguienteButton) {
                                                                                    siguienteButton.addEventListener('click', () => {
                                                                                        // Crear la card de "Final de Servicio"
                                                                                        const finalServicioCard = document.createElement('div');
                                                                                        finalServicioCard.className =
                                                                                            'rounded-lg shadow-md p-4 w-full sm:max-w-md mx-auto bg-[#fbe5e6]';
                                                                                        
                                                                                        // Botón Continuar (SOLO SI TIENE PERMISO)
                                                                                        const continuarHtml = window.permisosHelpDeskVisitas.puedeContinuar ? 
                                                                                            `<button class="flex items-center justify-center gap-2
                                                                      px-2 py-1 sm:px-3 sm:py-2 md:px-4 md:py-2.5
                                                                      text-xs sm:text-sm md:text-base
                                                                      font-semibold rounded-full shadow-md 
      ...
      `;
                                                                                        ...
                                                                                        // Agregar el evento de clic al botón "Finalizar" (SOLO SI TIENE PERMISO)
                                                                                        if (window.permisosHelpDeskVisitas.puedeContinuar) {
                                                                                            const finalizarServicioButton = document.getElementById(
                                                                                                `continueButton-${visita.idVisitas}`,
                                                                                            );
                                                                                            if (finalizarServicioButton) {
                                                                                                finalizarServicioButton.addEventListener('click', () => {
                                                                                                    // Mostrar el modal
                                                                                        ...
                                                                                        fetch(`/api/actualizarFechaLlegada/${visita.idVisitas}`, {
                                                                                            ...
                                                                                            .then((data) => {
                                                                                                if (data.success) {
                                                                                                    ...
                                                                                                    // Deshabilitar el botón "Siguiente" después de actualizar la fecha
                                                                                                    const siguienteButton = document.getElementById(
                                                                                                        `siguiente-${visita.idVisitas}`,
                                                                                                    );
                                                                                                    if (siguienteButton) {
                                                                                                        siguienteButton.disabled = true; // Deshabilitar el botón
                                                                                                        siguienteButton.classList.add('disabled'); // Opcional: agregar una clase CSS para estilos
                                                                                        ...
                                                                                        fetch('/api/seleccionar-visita-levantamiento', {
                                                                                            ...
                                                                                            headers: {
                                                                                                ...
                                                                                                'X-CSRF-TOKEN': document
                                                                                                    ...
                                                                                                    .getAttribute('content'), // CSRF Token si estás usando Blade en Laravel
                                                                                            },
                                                                                            body: JSON.stringify({
                                                                                                idTickets: idTicket,
                                                                                                idVisitas: idVisita,
                                                                                ...
                                                                                .then((data) => {
                                                                                    if (data.success) {
                                                                                        const uploadPhotoButton = document.getElementById(
                                                                                            `uploadPhotoButton-${visita.idVisitas}`,
                                                                                        );
                                                                                        const siguienteButton = document.getElementById(
                                                                                            `siguiente-${visita.idVisitas}`,
                                                                                        ); // Cambiar a "Siguiente"
                                                                                                            ...
                                                                                                            if (continueButton) {
                                                                                                                continueButton.addEventListener('click', () => {
                                                                                                                    ...
                                                                                                                    const modal = Alpine.$data(
                                                                                                                        document.querySelector('[x-data="modal"]'),
                                                                                                                    );
                                                                                                                    modal.toggle();
                                                                                                                });
                                                                                                            }
