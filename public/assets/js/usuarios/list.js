        ...
        initTable() {
            this.datatable = $('#myTable1').DataTable({
                ...
                columns: [
                    ...
                    { data: 'usuario', className: 'text-center' }, // AHORA SÍ EXISTE EN LA RESPUESTA
                    { data: 'correo', className: 'text-center' },
                    { data: 'tipoUsuario', className: 'text-center' },
                    { data: 'rol', className: 'text-center' },
                    { data: 'tipoArea', className: 'text-center' },
                    {
                        data: 'estado',
                        className: 'text-center',
                    ...
                    },
                    {
                        ...
                        render: tieneFirma => tieneFirma
                            ? `<svg class="w-6 h-6 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="#22c55e" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M4.293 12.293a1 1 0 011.414 0L10 16.586l8.293-8.293a1 1 0 011.414 1.414l-9 9a1 1 0 01-1.414 0l-5-5a1 1 0 010-1.414z" clip-rule="evenodd"/></svg>`
                            : `<svg class="w-6 h-6 text-red-500 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M12 2a10 10 0 100 20 10 10 0 000-20zm0 18a8 8 0 110-16 8 8 0 010 16zm4.707-10.707a1 1 0 00-1.414-1.414L12 10.586 9.707 8.293a1 1 0 10-1.414 1.414L10.586 12l-2.293 2.293a1 1 0 101.414 1.414L12 13.414l2.293 2.293a1 1 0 001.414-1.414L13.414 12l2.293-2.293z" clip-rule="evenodd"/></svg>`
                    },
                    {
                        data: null,
                        orderable: false,
                        ...
                        className: 'text-center',
                        render: (_, __, row) => {
                            // Verificar si permisos existe (por seguridad)
                            ...
                            if (permisos.puedeEditar) {
                                botones += `
                                    <a href="/usuario/${row.idUsuario}/edit" class="text-blue-600 hover:text-blue-800" title="Editar usuario">
                            ...
                            if (permisos.puedeActualizarEstado) {
                                botones += `
                                    <button type="button" class="text-gray-600 hover:text-gray-800" title="Cambiar Estado" onclick="toggleEstadoUsuario(${row.idUsuario}, ${row.estado})">
        ...
        if (result.isConfirmed) {
            fetch(`/api/usuarios/${idUsuario}/estado`, {
                ...
                .then(() => {
                    Swal.fire({
                        title: "¡Actualizado!",
                        text: `El estado del usuario ha sido cambiado a ${nuevoEstado === 1 ? 'activo' : 'inactivo'}.`,
                        icon: "success"
                    });
                    $('#myTable1').DataTable().ajax.reload(); // Recargar tabla sin recargar página
                })
                .catch(error => {
