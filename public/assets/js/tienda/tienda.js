document.addEventListener("alpine:init", () => {
    Alpine.data("multipleTable", () => ({
        datatable1: null,
        permisos: window.permisosTiendas, // Acceder a los permisos definidos en Blade

        init() {
            this.fetchDataAndInitTable();
            document.addEventListener('borrar', (e) => {
                this.deleteTienda(e.detail.id);
            });
        },
        ...
        async fetchDataAndInitTable() {
            this.datatable1 = $('#myTable1').DataTable({
                ...
                processing: true,
                ajax: {
                    url: '/api/tiendas/paginado',
                    type: 'GET'
                },
                columns: [
                    { data: 'ruc', className: 'text-center', render: ruc => `<div>${ruc || 'N/A'}</div>` },
                    { data: 'nombre', className: 'text-center', render: nombre => `<div>${nombre || 'N/A'}</div>` },
                    { data: 'celular', className: 'text-center', render: celular => `<div>${celular || 'N/A'}</div>` },
                    { data: 'email', className: 'text-center', render: email => `<div>${email || 'N/A'}</div>` },
                    {
                        data: 'direccion',
                    ...
                    },
                    {
                        ...
                        render: (_, __, row) => {
                            ...
                            if (this.permisos.puedeEditar) {
                                botones += `
                                    <a href="/tienda/${row.idTienda}/edit" class="ltr:mr-2 rtl:ml-2" x-tooltip="Editar">
                            ...
                            if (this.permisos.puedeEliminar) {
                                botones += `
                                    <button type="button" x-tooltip="Eliminar" @click="deleteTienda(${row.idTienda})">
        ...
        deleteTienda(idTienda) {
            ...
            if (!this.permisos.puedeEliminar) {
                Swal.fire('Error', 'No tienes permisos para eliminar tiendas.', 'error');
                return;
            }
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    .then(async res => {
                        ...
                        if (!res.ok) {
                            throw new Error(data.message || "No puedes eliminar, la tienda está asociada a otras entidades.");
                        }
