...
get filteredCotizaciones() {
    return this.cotizaciones.filter(cotizacion => {
        const matchesCodigo = !this.filtroCodigo || 
            cotizacion.codigo_cotizacion.toLowerCase().includes(this.filtroCodigo.toLowerCase());
        const matchesResponsable = !this.filtroResponsable || 
            cotizacion.responsable_cotizacion.toLowerCase().includes(this.filtroResponsable.toLowerCase());
        const matchesFecha = !this.filtroFecha || 
            cotizacion.fecha_cotizacion === this.filtroFecha;
...
agregarCotizacion() {
    ...
    const hasData = this.paramsTask.codigoCotizacion || 
...
agregarReunion() {
    // Validar que haya al menos un campo lleno
    const hasData = this.paramsTask.fechareunion || 
...
agregarLevantamiento() {
    // Validar que haya al menos un campo lleno
    const hasData = this.paramsTask.fecharequerimiento || 
...
agregarGanado() {
    // Validar que haya al menos un campo lleno
    const hasData = this.paramsTask.fechaganado || 
...
agregarObservado() {
    // Validar que haya al menos un campo lleno
    const hasData = this.paramsTask.fechaobservado || 
...
agregarRechazado() {
    // Validar que haya al menos un campo lleno
    const hasData = this.paramsTask.fecharechazo || 
...
get filteredLevantamientos() {
    return this.levantamientos.filter(levantamiento => {
        const matchesUbicacion = !this.filtroUbicacion || 
            (levantamiento.ubicacion && levantamiento.ubicacion.toLowerCase().includes(this.filtroUbicacion.toLowerCase()));
        const matchesFecha = !this.filtroFechalevantamiento || 
            levantamiento.fecha_requerimiento === this.filtroFechalevantamiento;
...
get filteredGanados() {
    return this.ganados.filter(ganado => {
        const matchesCodigo = !this.filtroCodigoGanado || 
            (ganado.codigo_cotizacion && ganado.codigo_cotizacion.toLowerCase().includes(this.filtroCodigoGanado.toLowerCase()));
        const matchesServicio = !this.filtroTipoServicio || 
            (ganado.tipo_servicio && ganado.tipo_servicio.toLowerCase().includes(this.filtroTipoServicio.toLowerCase()));
        const matchesFecha = !this.filtroFechaGanado || 
            ganado.fecha_ganado === this.filtroFechaGanado;
...
get filteredObservados() {
    return this.observados.filter(observado => {
        const matchesEstado = !this.filtroEstadoObservado || 
            (observado.estado_actual && observado.estado_actual.toLowerCase().includes(this.filtroEstadoObservado.toLowerCase()));
        const matchesFecha = !this.filtroFechaObservado || 
            observado.fecha_observado === this.filtroFechaObservado;
...
get filteredRechazados() {
    return this.rechazados.filter(rechazado => {
        const matchesMotivo = !this.filtroMotivoRechazo || 
            (rechazado.motivo_rechazo && rechazado.motivo_rechazo.toLowerCase().includes(this.filtroMotivoRechazo.toLowerCase()));
        const matchesFecha = !this.filtroFechaRechazo || 
            rechazado.fecha_rechazo === this.filtroFechaRechazo;
...
get filteredReuniones() {
    return this.reuniones.filter(reunion => {
        const matchesTipo = !this.filtroTipoReunion || 
            (reunion.tipo_reunion && reunion.tipo_reunion.toLowerCase().includes(this.filtroTipoReunion.toLowerCase()));
        const matchesResponsable = !this.filtroResponsableReunion || 
            (reunion.responsable_reunion && reunion.responsable_reunion.toLowerCase().includes(this.filtroResponsableReunion.toLowerCase()));
        const matchesFecha = !this.filtroFechaReunion || 
            reunion.fecha_reunion === this.filtroFechaReunion;
