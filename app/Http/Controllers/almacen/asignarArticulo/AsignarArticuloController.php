...
class AsignarArticuloController extends Controller
{
    ...
    public function create()
    {
        ...
        $articulos = Articulo::select('idArticulos', 'nombre', 'stock_total', 'maneja_serie', 'precio_venta', 'codigo_repuesto')
            ->where('estado', 1)
            ->where('stock_total', '>', 0)
            ->get()
            ->map(function ($articulo) {
                ...
                $articulo->texto_mostrar = $articulo->nombre ?: $articulo->codigo_repuesto;
    ...
    public function edit($id)
    {
        ...
        $articulos = Articulo::select([
            ...
            'articulos.nombre',
            'articulos.codigo_repuesto', // AGREGAR ESTE CAMPO
            ...
            ->groupBy('articulos.idArticulos', 'articulos.nombre', 'articulos.codigo_repuesto', 'articulos.stock_total', 'articulos.maneja_serie', 'articulos.precio_venta')
            ->get()
            ->map(function ($articulo) {
                // Asegurar que stock_disponible sea al menos 0
                $articulo->stock_disponible = max(0, $articulo->stock_disponible);

                // Crear texto para mostrar en el select
                $articulo->texto_mostrar = $articulo->nombre ?: $articulo->codigo_repuesto;
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'articulos' => 'required|array|min:1',
            'articulos.*.articulo_id' => 'required|exists:articulos,idArticulos', // Esto está bien
            'articulos.*.cantidad' => 'required|integer|min:1',
            'articulos.*.numero_serie' => 'nullable|string|max:255',
            ...
            foreach ($request->articulos as $articuloData) {
                ...
                if ($request->estado !== 'devuelto' && $articulo->stock_disponible < $articuloData['cantidad']) {
                    ...
                    if ($stockRealmenteDisponible < $articuloData['cantidad']) {
                        $nombreArticulo = $articulo->nombre ?: $articulo->codigo_repuesto;
                        throw new \Exception("Stock insuficiente para: {$nombreArticulo}. Stock disponible: {$stockRealmenteDisponible}");
                    }
                }
                ...
                if ($articulo->maneja_serie && empty($articuloData['numero_serie'])) {
                    $nombreArticulo = $articulo->nombre ?: $articulo->codigo_repuesto;
                    throw new \Exception("El artículo '{$nombreArticulo}' requiere número de serie");
                }
