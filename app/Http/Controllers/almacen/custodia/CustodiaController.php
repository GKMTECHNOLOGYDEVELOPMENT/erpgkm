    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('search') && !empty($request->search)) {
            ...
            $query->where(function ($q) use ($search) {
                // ✅ Buscar en campos directos de Custodia
                $q->where('codigocustodias', 'like', "%{$search}%")
                    ->orWhere('serie', 'like', "%{$search}%")
                    ->orWhere('numero_ticket', 'like', "%{$search}%");

                // ✅ Buscar en relación cliente (si existe)
                $q->orWhereHas('cliente', function ($clienteQuery) use ($search) {
                    $clienteQuery->where('nombre', 'like', "%{$search}%")
                        ->orWhere('documento', 'like', "%{$search}%");
                });

                // ✅ Buscar en marca (si existe)
                $q->orWhereHas('marca', function ($marcaQuery) use ($search) {
                    $marcaQuery->where('nombre', 'like', "%{$search}%");
                });

                // ✅ Buscar en modelo (si existe)
                $q->orWhereHas('modelo', function ($modeloQuery) use ($search) {
                    $modeloQuery->where('nombre', 'like', "%{$search}%");
                });

                // ✅ Buscar en ticket (SI TIENE TICKET)
                $q->orWhereHas('ticket', function ($ticketQuery) use ($search) {
                    $ticketQuery->where('numero_ticket', 'like', "%{$search}%")
                        ->orWhere('serie', 'like', "%{$search}%")
                        ...
                        ->orWhereHas('modelo', function ($modeloQuery) use ($search) {
                            $modeloQuery->where('nombre', 'like', "%{$search}%");
                        });
                });
            });
