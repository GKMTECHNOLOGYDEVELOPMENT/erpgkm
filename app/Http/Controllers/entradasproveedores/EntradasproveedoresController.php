    ...
    public function buscarProductoEntrada(Request $request)
    {
        ...
        try {
            $productos = DB::table('articulos as a')
                // Relación directa con modelo (uno a muchos)
                ->leftJoin('modelo as m_directo', 'a.idModelo', '=', 'm_directo.idModelo')
                ->leftJoin('marca as mr_directo', 'm_directo.idMarca', '=', 'mr_directo.idMarca')

                // Relación muchos a muchos a través de articulo_modelo
                ->leftJoin('articulo_modelo as am', 'a.idArticulos', '=', 'am.articulo_id')
                ->leftJoin('modelo as m_muchos', 'am.modelo_id', '=', 'm_muchos.idModelo')
                ->leftJoin('marca as mr_muchos', 'm_muchos.idMarca', '=', 'mr_muchos.idMarca')
                ...
                ->select(
                    ...
                    // Modelo y marca de relación directa
                    'm_directo.nombre as modelo_directo',
                    'mr_directo.nombre as marca_directo',

                    // Modelo y marca de relación muchos a muchos
                    'm_muchos.nombre as modelo_muchos',
                    'mr_muchos.nombre as marca_muchos',

                    'sc.nombre as subcategoria',

                    // Campo para mostrar en resultados (prioriza el modelo directo)
                    DB::raw("COALESCE(m_directo.nombre, m_muchos.nombre) as modelo"),
                    DB::raw("COALESCE(mr_directo.nombre, mr_muchos.nombre) as marca")
                )
                ->where('a.estado', 1)
                ->where(function ($queryBuilder) use ($query) {
                    $queryBuilder->where('a.nombre', 'LIKE', "%{$query}%")
                        // Buscar en modelo directo
                        ->orWhere('m_directo.nombre', 'LIKE', "%{$query}%")
                        ->orWhere('mr_directo.nombre', 'LIKE', "%{$query}%")
                        // Buscar en modelo muchos a muchos
                        ->orWhere('m_muchos.nombre', 'LIKE', "%{$query}%")
                        ->orWhere('mr_muchos.nombre', 'LIKE', "%{$query}%")
                ...
                ->distinct() // Evitar duplicados por la relación muchos a muchos
                ->orderBy('a.nombre')
                ->limit(50)
                ->get();

            Log::info('Resultados de la búsqueda con relaciones corregidas', [
    ...
    public function guardarEntradaProveedor(Request $request)
{
    try {
        ...
        // Obtener información del cliente general
        $clienteGeneral = null;
        $descripcionCliente = null;
        
        if ($request->cliente_general_id) {
            $clienteGeneral = DB::table('clientegeneral')
                ->where('idClienteGeneral', $request->cliente_general_id)
                ->first();
            $descripcionCliente = $clienteGeneral ? $clienteGeneral->descripcion : null;
        }

        // Generar código único para la entrada
        $codigoEntrada = $this->generarCodigoUnico();

        // Mapear tipos de entrada para CAS
        $tiposEntrada = [
            'consignacion' => 'CONSIGNACIÓN',
            'prestamo' => 'PRÉSTAMO',
            'muestra' => 'MUESTRA',
            'traslado' => 'TRASLADO INTERNO',
            'marca_asociada' => 'INGRESO POR MARCA ASOCIADA',
            'otro' => 'OTRO'
        ];

        // Construir el valor para CAS
        $tipoEntradaFormateado = $tiposEntrada[$request->tipo_entrada] ?? strtoupper($request->tipo_entrada);
        
        if ($descripcionCliente) {
            $cas = $tipoEntradaFormateado . ' ' . $descripcionCliente;
        } else {
            $cas = $tipoEntradaFormateado;
        }
        ...
        foreach ($productos as $producto) {
            ...
            // === CREAR O ACTUALIZAR KARDEX (AHORA CON CAS) ===
            $fechaIngreso = Carbon::parse($request->fecha_ingreso);
            $mes = $fechaIngreso->format('m');
            $anio = $fechaIngreso->format('Y');
            $clienteId = $request->cliente_general_id ?: null;
            ...
            if ($kardexExistente) {
                ...
                $kardexData = [
                    ...
                    'inventario_actual' => $nuevoInventarioActual,
                    'costo_inventario' => $costoInventarioPrevio + (($producto['precio_unitario'] ?? 0) * $cantidad),
                    'cas' => $cas, // ← AQUÍ SE AGREGA EL VALOR COMPUESTO
                    'created_at' => now(),
                    'updated_at' => now()
                ];
        ...
        return response()->json([
            ...
            'entrada_id' => $entradaId,
            'codigo_entrada' => $codigoEntrada,
            'cas_generado' => $cas // Para depuración
        ]);
    } catch (\Exception $e) {
        DB::rollback();
        Log::error('❌ ERROR AL GUARDAR ENTRADA DE PROVEEDOR: ' . $e->getMessage());
