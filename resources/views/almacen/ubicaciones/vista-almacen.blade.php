    ...
    <div x-data="almacenHeatmap" x-init="init()" class="container">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-xl md:text-2xl font-bold text-slate-800">
                            Vista del Almacen por SEDES
                        </h1>
                        <p class="mt-0.5 text-sm text-slate-500">
                            Analisis de actividad y ocupacion en tiempo real
                        </p>
                    </div>
                </div>
        ...
        <div class="panel rounded-xl shadow-sm border border-slate-200 p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div>
                    ...
                    <select x-model="mode" @change="updateChart()"
                        ...
                        <option value="fill">Por OcupaciÃ³n</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-3">
                ...
                @endif --}}                @if (\App\Helpers\PermisoHelper::tienePermiso('ACTUALIZAR RACK'))
        ...
        <div class="content-section space-y-4 mt-4">
                ...
                <div class="legend-horizontal flex flex-wrap items-center gap-6 justify-between">
                    ...
                    <div class="flex items-center gap-2 px-4 py-3 bg-white rounded-lg shadow-sm">
                        <div class="flex items-center justify-center w-6 h-6 bg-blue-100 rounded-full">
                            <span class="text-primary text-sm">ðŸŽ¨</span>
                        </div>
            ...
            <div class="text-center">
                <h3 class="text-xl font-bold text-gray-800">Mapa de Calor del AlmacÃ©n</h3>
                <p class="text-gray-600"
                    x-text="`Mostrando ${mode === 'heat' ? 'actividad' : 'ocupaciÃ³n'} - ${periodoLabel()}`"></p>
            </div>
            ...
            <div class="heatmap-full-section overflow-auto relative">
                ...
                <div x-show="loading" x-transition.opacity
                    ...
                    <div class="text-center">
                        <h3 class="text-xl font-bold text-slate-800 mb-2">Cargando almacÃ©n</h3>
                        <p class="text-slate-600 text-sm">Preparando visualizaciÃ³n en tiempo real...</p>
                    </div>
                ...
                <div x-show="modalCrearRack.open" x-transition x-transition.duration.300
                    ...
                    <div class="p-5">
                        <form @submit.prevent="crearRack()" class="space-y-4">
                            ...
                            <!-- Nombre del Rack con Sugerencia AutomÃ¡tica -->
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-2 dark:text-white-dark/70">
                                    ...
                                    <span x-show="modalCrearRack.sugerencia" class="text-green-600 text-xs ml-2">
                                        ðŸ’¡ Sugerencia: <span x-text="modalCrearRack.sugerencia"
                            ...
                            <div x-show="modalEditarDimensiones.cambiosDetectados"
                                ...
                                <div :class="modalEditarDimensiones.puedeActualizar ? 'text-amber-800' : 'text-red-800'"
                                    ...
                                    <div x-show="!modalEditarDimensiones.puedeActualizar" class="mt-2 text-xs">
                                        âš ï¸ Para disminuir dimensiones, primero vacÃ­e las ubicaciones que serÃ­an
                                        eliminadas.
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('almacenHeatmap', () => ({
                ...
                // MÃ©todo para abrir modal de selecciÃ³n de rack
                ...
                // MÃ©todo para seleccionar un rack y abrir el modal de ediciÃ³n
                async seleccionarRackParaEdicion(rack) {
                    ...
                    // Abrir modal de ediciÃ³n con el rack seleccionado
                ...
                calcularCambios() {
                    ...
                    // Calcular ubicaciones que se eliminarÃ­an
                    ...
                    if (diferencia > 0) {
                        this.modalEditarDimensiones.resumenCambios =
                            `âœ… Se generarÃ¡n ${diferencia} nuevas ubicaciones. Total: ${ubicacionesNuevas} ubicaciones.`;
                        this.modalEditarDimensiones.puedeActualizar = true;
                    } else if (diferencia < 0) {
                        if (!puedeDisminuir) {
                            this.modalEditarDimensiones.resumenCambios =
                                `âŒ Se eliminarÃ­an ${ubicacionesAEliminar} ubicaciones, pero hay productos en algunas de ellas.`;
                            this.modalEditarDimensiones.puedeActualizar = false;
                        } else {
                            this.modalEditarDimensiones.resumenCambios =
                                `âš ï¸ Se eliminarÃ¡n ${ubicacionesAEliminar} ubicaciones. Total: ${ubicacionesNuevas} ubicaciones.`;
                            this.modalEditarDimensiones.puedeActualizar = true;
                        }
                    } else {
                        this.modalEditarDimensiones.resumenCambios =
                            'No hay cambios en el nÃºmero de ubicaciones.';
                        this.modalEditarDimensiones.puedeActualizar = true;
                    }
                },

                // MÃ©todo para actualizar dimensiones
                ...
                mostrarMensajeTemporal(mensaje, tipo = 'info') {
                    // Puedes implementar tu sistema de notificaciones aquÃ­
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            if (result.data && result.data.total_ubicaciones) {
                                console.log(
                                    `Se crearon ${result.data.total_ubicaciones} ubicaciones automÃ¡ticamente`
                                );
                            }
                        } else {
                ...
                // MÃ©todo para abrir modal de ediciÃ³n
                async abrirModalEditarDimensiones(rack) {
                    try {
                        ...
                        if (result.success) {
                            ...
                            this.error('Error al cargar informaciÃ³n del rack');
                        }
                    } catch (error) {
                ...
                // MÃ©todo para verificar si el rack tiene productos
                async verificarProductosEnRack(rackId) {
                    try {
                        ...
                        const result = await response.json();
                        console.log('Respuesta verificaciÃ³n productos:', result);
                ...
                async cargarDatos() {
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            alert('Error al cargar los datos del almacÃ©n');
                        }
                    } catch (error) {
                ...
                periodoLabel() {
                    return 'Estado actual del almacÃ©n';
                },
                ...
                // âœ… NUEVAS FUNCIONES PARA ESTADOS
                getIconByCantidad(cantidad) {
                    if (cantidad === 0) return "âšª";
                    if (cantidad <= 100) return "ðŸŸ¢";
                    if (cantidad <= 500) return "ðŸŸ¡";
                    if (cantidad <= 1000) return "ðŸŸ ";
                    return "ðŸ”´";
                },
        ...
        `;
                ...
                baseOption() {
                    ...
                    // âœ… CAMBIADO: Usar cantidad en lugar de value/ocupacion
                    const data = this.data.map(d => [
                        d.x || 0,
                        d.y || 0,
                        d.cantidad || 0, // â† Ahora usamos cantidad absoluta
                        d.ubicacion || 'N/A',
                        d.piso || 1,
                        d.rack || 'N/A',
                        d.cantidad || 0, // â† Mantenemos cantidad aquÃ­ tambiÃ©n
                        d.categoria || 'Sin categorÃ­a',
                        d.sede || 'N/A',
                        d.tipo_articulo || 'Sin tipo',
                        d.nivel || 1
                    ]);

                    // OBTENER DATOS DEL YAXIS CON VALIDACIÃ“N
                    ...
                    const yAxisDataWithStyle = yAxisData.map(yValue => {
                        ...
                        // ASIGNAR ESTILOS SEGÃšN TIPO DE RACK
                        if (tipoRack === 'spark') {
                            return {
                                value: letra,
                                textStyle: {
                        ...
                        tooltip: {
                            ...
                            formatter: (p) => {
                                try {
                                    ...
                                    // âœ… CAMBIADO: Obtener estado basado en cantidad
            ...
            <div style="padding:12px; min-width: 320px;">
                ...
                <div style="margin-bottom:6px;">ðŸ“ <strong>UbicaciÃ³n:</strong> ${ubicacion}</div>
                <div style="margin-bottom:6px;">ðŸ·ï¸ <strong>CategorÃ­a:</strong> ${categoria}</div>
                <div style="margin-bottom:6px;">ðŸ“Š <strong>Cantidad:</strong> ${cantidadTotal} unidades</div>
                <div style="margin-bottom:6px;">ðŸ“ˆ <strong>Estado:</strong> ${icono} ${estado}</div>
                <div style="margin-bottom:6px;">ðŸ”§ <strong>Tipo ArtÃ­culo:</strong> ${tipoArticulo}</div>
                <div style="margin-bottom:6px;">ðŸ—ï¸ <strong>Piso:</strong> ${piso}</div>
                
                <div style="font-size:13px;color:#94a3b8;margin-top:10px;">${this.periodoLabel()}</div>
                <div style="font-size:13px;color:#fbbf24;margin-top:6px;">ðŸ’¡ Click para ver detalles</div>
            </div>
        `;
                                } catch (error) {
                                    console.error('Error en tooltip:', error);
                                    return '<div>Error al cargar informaciÃ³n</div>';
                                }
                            }
                        },
                        grid: {
                        ...
                        series: [{
                            ...
                            itemStyle: {
                                // âœ… MANTENIDO: Seguimos usando getFillColorByFloor para los colores de fondo
                                ...
                                if (rack && sede && rack !== 'N/A' && sede !== 'N/A') {
                                    ...
                                    try {
                                        ...
                                        const tipoRack = result.success ? result.data
                                            .tipo_rack : 'spark';
                                        console.log('ðŸŽ¯ Tipo de rack obtenido:', tipoRack);
                                        ...
                                        console.error('Error al obtener tipo de rack:',
                                            error);
                                        // Fallback tambiÃ©n en nueva ventana
                                        const url =
