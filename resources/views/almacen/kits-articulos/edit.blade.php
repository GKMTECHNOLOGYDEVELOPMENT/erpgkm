<x-layout.default>
  ...
  <div>
        <ul class="flex space-x-2 rtl:space-x-reverse">
            ...
            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                <span>Editar Kit: {{ $kit->nombre }}</span>
            </li>
        </ul>
    </div>
    ...
    <div class="panel mt-6 p-5 max-w-6x2 mx-auto">
        <h2 class="text-xl font-bold mb-5 flex items-center">
            <i class="fas fa-boxes text-primary mr-2"></i> Editar Kit de Productos
        </h2>
    <form id="kitForm" method="POST" enctype="multipart/form-data" action="{{ route('almacen.kits.update', $kit->idKit) }}" data-kit-id="{{ $kit->idKit }}">
    @csrf
    @method('PUT')
                ...
                <div class="relative">
                    ...
                    <div class="relative mt-1">
                        ...
                        <input id="codigo_barras" name="codigo_barras" type="text" class="clean-input w-full"
                            placeholder="Ingrese código de barras" value="{{ old('codigo_barras', $kit->codigo) }}" required>
                    </div>
                </div>
                ...
                <div class="relative">
                    ...
                    <div class="relative mt-1">
                        ...
                        <input id="sku" name="sku" type="text" class="clean-input w-full" 
                            placeholder="Ingrese SKU" value="{{ old('sku', $kit->sku) }}">
                    </div>
                </div>
                ...
                <div class="relative">
                    ...
                    <div class="relative mt-1">
                        ...
                        <input id="nombre" name="nombre" type="text" class="clean-input w-full" 
                            placeholder="Ingrese nombre del kit" value="{{ old('nombre', $kit->nombre) }}" required>
                    </div>
                </div>
                ...
                <div class="relative">
                    ...
                    <div class="relative mt-1">
                        ...
                        <input id="stock_total" name="stock_total" type="number" min="0" class="clean-input w-full"
                            placeholder="Ingrese stock total" value="{{ old('stock_total', $kit->stock_total) }}" required>
                    </div>
                </div>
                ...
                <div class="relative">
                    ...
                    <div class="relative mt-1">
                        ...
                        <input id="stock_minimo" name="stock_minimo" type="number" min="0" class="clean-input w-full"
                            placeholder="Ingrese stock mínimo" value="{{ old('stock_minimo', $kit->stock_minimo) }}" required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center mt-1">
                        <button type="button" id="toggleMonedaVenta"
                            ...
                            <span id="precio_venta_symbol" class="w-8 text-center">
                                {{ $monedas->firstWhere('idMonedas', $kit->monedaVenta)->nombre ?? 'S/' }}
                            </span>
                        </button>
                        <div class="relative flex-1">
                            <input id="precio_venta" name="precio_venta" type="number" step="0.01" min="0"
                                class="clean-input w-full" placeholder="0.00" 
                                value="{{ old('precio_venta', $kit->precio_venta) }}" required>
                        </div>
                        <input type="hidden" id="moneda_venta" name="moneda_venta" value="{{ $kit->monedaVenta }}">
                    </div>
                </div>

                <!-- Foto -->
                <div class="mb-5" x-data="{ 
                    fotoPreview: '{{ $kit->foto ? 'data:image/jpeg;base64,'.base64_encode($kit->foto) : '/assets/images/articulo/producto-default.png' }}', 
            ...
            <div class="mt-8">
                    ...
                    <div class="overflow-x-auto">
                        <table class="productos-kit-table">
                            ...
                            <tbody id="productos-kit-body">
                                <!-- Los productos existentes se cargarán con JavaScript -->
                            </tbody>
                            <tfoot>
                                <tr>
                ...
                <input type="hidden" id="productos_kit" name="productos_kit" value="{{ json_encode($productosKit) }}">
            </div>
            ...
            <div class="flex justify-end mt-6 gap-4">
                ...
                <!-- Guardar -->
                <button type="submit" id="btnGuardar" class="btn btn-primary flex items-center">
                    <i class="fas fa-save mr-2"></i> Actualizar Kit
                </button>
            </div>
        </form>
    </div>
  ...
  document.addEventListener("DOMContentLoaded", function () {
        ...
        let productosKit = JSON.parse(document.getElementById('productos_kit').value || '[]');
        let totalKit = 0;
        ...
        function cargarProductosExistentes() {
            ...
            productosKit.forEach(producto => {
                ...
                row.innerHTML = `
                    ...
                    <td id="subtotal-${producto.id}">S/ ${producto.subtotal.toFixed(2)}</td>
                    <td class="text-center">
                        <button type="button" onclick="eliminarProductoKit('${producto.id}')" class="btn-eliminar-producto">
    ...
    // ---------------------------
// 5. Envío del formulario por AJAX para actualizar
// ---------------------------
document.getElementById("btnGuardar").addEventListener("click", function () {
    const form = document.getElementById("kitForm");
    const kitId = form.getAttribute('data-kit-id'); // Obtener el ID del kit
    ...
    // Usar la ruta definida en tu web.php (almacen.kits.update)
    fetch(`/kits/update/${kitId}`, {
    ...
    .then(data => {
        if (data.success) {
            toastr.success("Kit actualizado correctamente");
            
            // Redirigir después de 2 segundos
            setTimeout(() => {
                window.location.href = "/kits"; // O usa route('almacen.kits.index')
            }, 2000);
        } else {
            toastr.error(data.message || "Ocurrió un error al actualizar el kit");
            console.error(data);
        }
    })
    .catch(error => {
        console.error('Error:', error);
        toastr.error(error.message || "Error en la comunicación con el servidor.");
