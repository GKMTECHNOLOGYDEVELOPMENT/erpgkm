    ...
    <div class="mx-auto w-full px-4 py-6">
        ...
        <div class="panel rounded-xl shadow-lg border-0 mb-6">
            <div class="p-5">
                ...
                <form method="GET" action="{{ route('asignar-articulos.index') }}"
                                    ...
                                    <div
                                        ...
                                        @php
                                            $iniciales =
                                                ...
                                                substr($usuario->apellidoPaterno, 0, 1);
                                        @endphp
                                        <span class="text-white text-xl font-bold">{{ $iniciales }}</span>
                                    </div>
                                    <div
                                        class="absolute -bottom-1 -right-1 w-5 h-5 bg-{{ $asignacion->estado == 'activo' ? 'success' : ($asignacion->estado == 'devuelto' ? 'secondary' : 'danger') }} rounded-full border-2 border-white">
                                    </div>
                                </div>
                                <div>
                                    <h3 class="text-lg font-bold text-white">{{ $usuario->Nombre }}
                                        {{ $usuario->apellidoPaterno }}</h3>
                                    <p class="text-blue-100 text-xs">{{ $usuario->correo }}</p>
                                </div>
                            </div>
                            <span
                        ...
                        <div class="space-y-3 h-32 overflow-y-auto">
                            @foreach ($detalles as $detalle)
                                @php
                                    ...
                                    // Determinar qué mostrar: nombre o codigo_repuesto
                                    $displayText = $articulo->nombre ?: $articulo->codigo_repuesto;
                                @endphp
                                <div
                                    ...
                                    <div class="flex items-center space-x-3 flex-1">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-start justify-between">
                                                <div class="min-w-0">
                                                    <!-- Mostrar nombre o código -->
                                                    <p class="font-medium text-gray-900 truncate">
                                                        {{ $displayText }}

                                                        <!-- Mostrar código entre paréntesis si existe y es diferente al nombre -->
                                                        @if ($articulo->codigo_repuesto && $articulo->nombre && $articulo->codigo_repuesto != $articulo->nombre)
                                                            <span
                                                                class="text-sm text-gray-500 ml-2">({{ $articulo->codigo_repuesto }})</span>
                                                        @endif
                                                    </p>
                                                    ...
                                                    <div class="flex flex-col space-y-1 mt-1">
                                                        <div class="flex items-center space-x-3 flex-wrap gap-1">
                                                            ...
                                                            <!-- Si NO tiene número de serie, mostrar fechas en línea -->
                                                            @if (!$detalle->numero_serie)
                                                                <span class="text-xs text-gray-500">
                                                        ...
                                                        <!-- Si TIENE número de serie, mostrar fechas debajo -->
                                                        @if ($detalle->numero_serie)
                                    ...
                                    <div class="ml-3 flex-shrink-0">
                                        <span
                                            class="bg-{{ $detalle->estado_articulo == 'activo' ? 'success' : ($detalle->estado_articulo == 'dañado' ? 'danger' : 'warning') }}/10 text-{{ $detalle->estado_articulo == 'activo' ? 'success' : ($detalle->estado_articulo == 'dañado' ? 'danger' : 'warning') }} text-xs font-semibold px-2 py-1 rounded-full whitespace-nowrap">
                        ...
                        <div class="flex space-x-3">
                            @if ($asignacion->estado == 'activo')
                                <form action="{{ route('asignar-articulos.devolver', $asignacion->id) }}"
                                    ...
                                    @csrf
                                    <button type="submit"
                                        class="w-full px-4 py-2.5 bg-success text-white font-semibold rounded-xl hover:bg-success-dark transition-all duration-200 shadow-md hover:shadow-lg flex items-center justify-center">
                            ...
                            <a href="{{ route('asignar-articulos.edit', $asignacion->id) }}" class="flex-1">
                                <button
                                    class="w-full px-4 py-2.5 bg-primary text-white font-semibold rounded-xl hover:bg-primary-dark transition-all duration-200 shadow-md hover:shadow-lg flex items-center justify-center">
            ...
            @empty
                <div class="col-span-full">
                    <div class="panel bg-white rounded-xl shadow-lg border-0 p-8 text-center">
                        ...
                        <p class="text-gray-600 mb-6">No se encontraron asignaciones de artículos. Crea una nueva
                            asignación para empezar.</p>
                        <a href="{{ route('asignar-articulos.create') }}"
                            class="px-5 py-2.5 bg-primary text-white font-semibold rounded-lg hover:bg-primary-dark transition-all duration-200 shadow-md hover:shadow-lg inline-flex items-center no-underline">
            ...
            <div class="mt-6">
                <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                    <div class="text-sm text-gray-600">
                        Mostrando {{ $asignaciones->firstItem() }} - {{ $asignaciones->lastItem() }} de
                    ...
                    <nav class="flex items-center space-x-1">
                        ...
                        @else
                            <a href="{{ $asignaciones->url(1) }}"
                                class="px-3 py-1.5 bg-white text-gray-700 rounded-lg hover:bg-primary hover:text-white transition-all duration-200 border border-gray-200 no-underline">
                                <i class="fas fa-chevron-left"></i>
                            </a>
                        @endif
                        ...
                        @else
                            <a href="{{ $asignaciones->previousPageUrl() }}"
                                class="px-3 py-1.5 bg-white text-gray-700 rounded-lg hover:bg-primary hover:text-white transition-all duration-200 border border-gray-200 no-underline">
                                <i class="fas fa-chevron-left"></i>
                            </a>
                        @endif
                        ...
                        @if ($asignaciones->hasMorePages())
                            <a href="{{ $asignaciones->nextPageUrl() }}"
                                class="px-3 py-1.5 bg-white text-gray-700 rounded-lg hover:bg-primary hover:text-white transition-all duration-200 border border-gray-200 no-underline">
                        ...
                        <!-- Última página -->
                        @if ($asignaciones->hasMorePages())
                            <a href="{{ $asignaciones->url($asignaciones->lastPage()) }}"
                                class="px-3 py-1.5 bg-white text-gray-700 rounded-lg hover:bg-primary hover:text-white transition-all duration-200 border border-gray-200 no-underline">
                                <i class="fas fa-chevron-right"></i>
                            </a>
                        @else
