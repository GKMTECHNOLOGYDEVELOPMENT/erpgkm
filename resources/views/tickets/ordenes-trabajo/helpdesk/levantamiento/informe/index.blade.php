@php
    $rutaPdf = route('ordenes.helpdesk.pdf.levantamiento', ['idOt' => $orden->idTickets]);
@endphp
...
<div class="mt-4 border rounded-lg overflow-hidden">
    <div id="pdfContainer">
        <iframe id="informePdfFrame"
            data-src="{{ route('ordenes.helpdesk.pdf.levantamiento', ['idOt' => $orden->idTickets]) }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            window.verificarCambios = function() {
                fetch("/ordenes/{{ $orden->idTickets }}/check-updates")
                    ...
                    .then(data => {
                        if (data.ultimaActualizacion && data.ultimaActualizacion !== ultimaActualizacion) {
                            ...
                            window.cargarPdfDesdeAlpine?.(); // ✅ usamos función global
                        }
                    })
