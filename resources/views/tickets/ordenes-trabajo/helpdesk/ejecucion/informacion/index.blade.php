...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
    ...
    <div class="p-5 rounded-lg shadow-md">
        ...
        <div class="grid grid-cols-1 gap-4 mt-4">
            ...
            <div>
                ...
                <textarea id="justificacion" name="justificacion" rows="3" class="form-input w-full mt-2" data-toggle="tooltip"
                    data-tooltip="Describe el motivo del cambio de estado"></textarea>
            </div>

            <!-- Botón Guardar -->
            <div class="flex justify-end">
                @if (\App\Helpers\PermisoHelper::tienePermiso('GUARDAR DETALLES DE ESTADOS HELP DESK EJECUCION'))
    ...
    <div class="p-5 rounded-lg shadow-md">
        ...
        <div class="flex justify-end mt-6">
            @if (\App\Helpers\PermisoHelper::tienePermiso('GUARDAR INVENTARIO HELP DESK EJECUCION'))
            ...
            <form method="POST" id="formInstalar" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="serieInstalar" class="form-input w-full"
                        placeholder="Ingrese Nro. de Serie" data-toggle="tooltip"
                        data-tooltip="Ingrese el número de serie único del producto">
                </div>
                ...
                <!-- Botón Guardar -->
                <div class="flex justify-end">
                    @if (\App\Helpers\PermisoHelper::tienePermiso('GUARDAR EQUIPOS INSTALAR HELP DESK EJECUCION'))
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        <div class="lg:col-span-1 p-6 rounded-2xl shadow-md bg-white dark:bg-slate-800 space-y-6">
            ...
            <form method="POST" id="formRetirar" class="space-y-4">
                ...
                <!-- Botón Guardar -->
                <div class="flex justify-end">
                    @if (\App\Helpers\PermisoHelper::tienePermiso('GUARDAR EQUIPOS RETIRAR HELP DESK EJECUCION'))
...
<div id="modalEditarProducto"
    class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center hidden z-50 px-4">
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-4 w-full max-w-sm">
        ...
        <form id="formEditarProducto" class="space-y-3">
            ...
            <div class="grid grid-cols-1 gap-3">
                <!-- Tipo Producto -->
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Tipo
                        Producto</label>
                ...
                <!-- Número de Serie -->
                <div>
                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Nro. de
                        Serie</label>
                    <input type="text" id="editSerie" name="numeroSerie" class="form-input text-xs h-8 w-full"
                ...
                <div>
                    <label
                        class="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Observaciones</label>
            ...
            <div class="flex justify-end space-x-2 pt-2">
                <button type="button" id="cancelarEdicion"
                    class="btn btn-secondary text-xs px-3 py-1.5">Cancelar</button>
...
<script>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response && response.length) {
                    ...
                    $selectModelo.append(
                        '<option value="" disabled selected>No hay modelos disponibles</option>');
                }
            },
            error: function(xhr) {
    ...
    $(document).ready(function() {
        ...
        inicializarSelects();
        $(document).ready(function() {
            // Inicializar el select de estado con Select2
            $("#estado").select2({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        toastr.error(response.message ||
                            "Hubo un error al guardar el equipo.");
                    }
                },
                error: function() {
                    ocultarLoading(btn, textoOriginal);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            toastr.error(response.message ||
                                `Hubo un error al eliminar el producto ${tipo}.`);
                        }
                    },
                    error: function() {
                        toastr.error(
                            `Hubo un error al procesar la solicitud de eliminación.`);
                    }
                });
            }
        });
