                ...
                <div class="lg:w-1/2 w-full lg:max-w-fit">
                    ...
                    <div class="flex items-center mt-4">
                        ...
                        <input id="tipo" type="text" name="tipo"
                            class="form-input lg:w-[250px] w-2/3" placeholder="Constancia de Entrega"
                            value="Constancia de Entrega" />
                    </div>

                    <div class="flex items-center mt-4">
                        <label for="fechacompra" class="flex-1 ltr:mr-2 rtl:ml-2 mb-0">Fecha Compra</label>
                        <input id="fechacompra" type="date" name="fechacompra" class="form-input lg:w-[250px] w-2/3"
                ...
                <div class="flex justify-between lg:flex-row flex-col">
                    <div class="lg:w-1/2 w-full ltr:lg:mr-6 rtl:lg:ml-6 mb-6">
                        ...
                        <div class="mt-4 flex items-center">
                            ...
                            <input id="emailcliente" type="email" name="emailcliente"
                                class="form-input flex-1" value="{{ optional($orden->cliente)->email }}" placeholder="Enter Email" />
                        </div>
                        <div class="mt-4 flex items-center">
                            ...
                            <input id="direccioncliente" type="text" name="direccioncliente"
                                class="form-input flex-1" value="{{ optional($orden->cliente)->direccion }}" placeholder="Enter Address" />
                        </div>
                        <div class="mt-4 flex items-center">
        ...
        <div>
            ...
            <template x-if="params.fotosExistentes.length > 0 || params.photos.length > 0">
                <div class="mt-4 grid grid-cols-3 gap-2">
                    <!-- Fotos existentes -->
                    <template x-for="(foto, index) in params.fotosExistentes" :key="'existente-'+index">
                        <div class="relative">
                            <img :src="foto.preview" class="h-24 w-full rounded object-cover border">
                            <button type="button" @click="eliminarFotoExistente(index)"
                                class="absolute top-0 right-0 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center">
                                ×
                            </button>
                            <div class="absolute bottom-0 left-0 bg-black bg-opacity-50 text-white text-xs p-1 w-full">
                                Existente
                            </div>
                        </div>
                    </template>
                    ...
                    <template x-for="(photo, index) in params.photos" :key="'nueva-'+index">
                        <div class="relative">
                            <img :src="photo.preview" class="h-24 w-full rounded object-cover border">
            ...
            if (data.success && data.constancia) {
                ...
                if (data.constancia.fotos && data.constancia.fotos.length > 0) {
                    this.params.fotosExistentes = data.constancia.fotos.map(foto => ({
                        id: foto.idfoto,
                        preview: foto.imagen_url, // Asegúrate que tu API devuelva esta URL
                        esExistente: true
                    }));
                }
            }
...
async eliminarFotoDelServidor(fotoId) {
    ...
    const submitBtn = this.$refs.submitBtn || document.querySelector('#constanciaForm button[type="submit"]');
    ...
    formData.append('fotos_existentes', JSON.stringify(fotosExistentesIds));
        ...
        const btn = this.$refs.submitBtn || document.querySelector('#constanciaForm button[type="submit"]');
        if (btn) {
            btn.innerHTML = `
