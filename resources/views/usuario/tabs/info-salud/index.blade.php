        ...
        <div class="border border-[#ebedf2] dark:border-[#191e3a] rounded-md p-4 bg-white dark:bg-[#0e1726]">
            ...
            <div class="flex justify-end mb-4">
                <button type="button" id="add-familiar-btn"
                    class="btn btn-primary border-none px-4 py-2 flex items-center gap-2 rounded-lg">
            ...
            <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                <table class="w-full text-sm">
                    <thead class="bg-pink-50 dark:bg-pink-900/20 border-b border-pink-200 dark:border-pink-900/30">
                        <tr>
                            <th
                                class="px-4 py-3 text-left text-xs font-semibold text-pink-800 dark:text-pink-300 uppercase">
                                Parentesco</th>
                            <th
                                class="px-4 py-3 text-left text-xs font-semibold text-pink-800 dark:text-pink-300 uppercase">
                            ...
                            <th
                                ...
                                Domicilio</th>
                            <th
                                class="px-4 py-3 text-center text-xs font-semibold text-pink-800 dark:text-pink-300 uppercase">
                                Acciones</th>
                        </tr>
                    </thead>
                    <tbody id="familiares-tbody" class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($familiares as $familiar)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
                                data-id="{{ $familiar->idFamiliar }}">
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">
                                    {{ $familiar->parentesco ?? 'N/A' }}</td>
                                <td class="px-4 py-3 font-medium text-gray-800 dark:text-gray-200">
                                    {{ $familiar->apellidosNombres ?? 'N/A' }}</td>
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">
                                    {{ $familiar->nroDocumento ?? 'N/A' }}</td>
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">
                                    {{ $familiar->ocupacion ?? 'N/A' }}</td>
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $familiar->sexo ?? 'N/A' }}
                                </td>
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">
                                    {{ $familiar->fechaNacimiento ? date('d/m/Y', strtotime($familiar->fechaNacimiento)) : 'N/A' }}
                                </td>
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-300">
                                    {{ $familiar->domicilioActual ?? 'N/A' }}</td>
                                <td class="px-4 py-3 text-center">
                                    <div class="flex justify-center items-center gap-2">
                                        <button class="text-blue-600 hover:text-blue-800 edit-familiar"
                                            data-id="{{ $familiar->idFamiliar }}" title="Editar">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button class="text-red-600 hover:text-red-800 delete-familiar"
                                            data-id="{{ $familiar->idFamiliar }}" title="Eliminar">
        ...
        <div class="border border-[#ebedf2] dark:border-[#191e3a] rounded-md p-4 bg-white dark:bg-[#0e1726]">
            ...
            <div class="mb-5 pb-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1">¿Ha recibido la
                            vacuna?</label>
                        <div class="flex gap-4">
                            ...
                            <label class="flex items-center gap-1">
                                <input type="radio" name="vacuna_covid" value="1"
                                    class="w-3.5 h-3.5 text-red-600 vacuna-radio"
                                    {{ $vacunaCovid === true ? 'checked' : '' }}> Sí
                            </label>
                            <label class="flex items-center gap-1">
                                <input type="radio" name="vacuna_covid" value="0"
                                    class="w-3.5 h-3.5 text-red-600 vacuna-radio"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" id="covid_dosis1" name="covid_dosis1"
                                class="flatpickr form-input w-full text-sm campo-vacuna" placeholder="Fecha"
                                value="{{ $covidDosis1 ?? '' }}" {{ $vacunaCovid === false ? 'disabled' : '' }}>
                            <i
                                class="fas fa-calendar absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" id="covid_dosis2" name="covid_dosis2"
                                class="flatpickr form-input w-full text-sm campo-vacuna" placeholder="Fecha"
                                value="{{ $covidDosis2 ?? '' }}" {{ $vacunaCovid === false ? 'disabled' : '' }}>
                            <i
                                class="fas fa-calendar absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" id="covid_dosis3" name="covid_dosis3"
                                class="flatpickr form-input w-full text-sm campo-vacuna" placeholder="Fecha"
                                value="{{ $covidDosis3 ?? '' }}" {{ $vacunaCovid === false ? 'disabled' : '' }}>
                            <i
                                class="fas fa-calendar absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
                        </div>
                    </div>
            ...
            <div class="mb-5 pb-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1">¿Padece alguna
                            dolencia crónica?</label>
                        <div class="flex gap-4">
                            @php
                                $dolenciaCronica = $salud->dolenciaCronica ?? null;
                            @endphp
                            <label class="flex items-center gap-1">
                                <input type="radio" name="dolencia_cronica" value="1"
                                    class="w-3.5 h-3.5 text-orange-600 dolencia-radio"
                                    {{ $dolenciaCronica === true ? 'checked' : '' }}> Sí
                            </label>
                            <label class="flex items-center gap-1">
                                <input type="radio" name="dolencia_cronica" value="0"
                                    class="w-3.5 h-3.5 text-orange-600 dolencia-radio"
                    ...
                    <div class="md:col-span-2">
                        <label
                            class="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1">Especificar</label>
                        <input type="text" name="dolencia_detalle" id="dolencia_detalle"
                            class="form-input w-full text-sm campo-dolencia"
            ...
            <div class="mb-5 pb-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="flex gap-4">
                            ...
                            <label class="flex items-center gap-1">
                                <input type="radio" name="discapacidad" value="1"
                                    class="w-3.5 h-3.5 text-blue-600 discapacidad-radio"
                    ...
                    <div class="md:col-span-2">
                        <label
                            class="text-xs font-medium text-gray-600 dark:text-gray-400 block mb-1">Especificar</label>
                        <input type="text" name="discapacidad_detalle" id="discapacidad_detalle"
                            class="form-input w-full text-sm campo-discapacidad"
            ...
            <div class="mb-5 pb-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-4 sm:grid-cols-8 gap-2 max-w-2xl">
                    @foreach (['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'] as $sangre)
                        <label
                            class="flex flex-col items-center p-2 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer">
                            <input type="radio" name="tipo_sangre" value="{{ $sangre }}"
                                class="w-3.5 h-3.5 text-red-600 mb-1" {{ $tipoSangre == $sangre ? 'checked' : '' }}>
            ...
            <div class="mt-6">
                ...
                <div class="flex justify-end mb-4">
                    <button type="button" id="add-contacto-btn"
                        class="btn bg-warning text-white border-none px-4 py-2 flex items-center gap-2 rounded-lg">
                        <i class="fas fa-plus"></i>
                ...
                <div id="contactos-container" class="space-y-3">
                    @forelse($contactosEmergencia as $contacto)
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-white dark:bg-[#0e1726] border border-gray-200 dark:border-gray-700 rounded-lg relative group"
                            ...
                            <button class="absolute top-2 right-2 text-gray-400 hover:text-red-600 delete-contacto"
                                data-id="{{ $contacto->idContacto }}" title="Eliminar">
                            ...
                            <div>
                                ...
                                <input type="text" class="form-input w-full text-sm contacto-nombre"
                                    value="{{ $contacto->apellidosNombres }}" placeholder="Nombre completo"
                                    data-id="{{ $contacto->idContacto }}">
                            </div>
                            <div>
                                <label class="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">
                                    <i class="fas fa-heart mr-1 text-red-500"></i> Parentesco
                                </label>
                                <input type="text" class="form-input w-full text-sm contacto-parentesco"
                                    value="{{ $contacto->parentesco }}" placeholder="Ej: Padre, Madre"
                                    data-id="{{ $contacto->idContacto }}">
                            </div>
                            <div>
                                <label class="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">
                                    <i class="fas fa-phone-alt mr-1 text-green-500"></i> <i
                                        class="fas fa-map-marker-alt mr-1 text-yellow-600"></i> Teléfono / Dirección
                                </label>
                                <input type="text" class="form-input w-full text-sm contacto-direccion"
                                    value="{{ $contacto->direccionTelefono }}"
                                    placeholder="Ej: 987654321 - Calle 123" data-id="{{ $contacto->idContacto }}">
                            </div>
                        </div>
                    @empty
                        <div
                            class="text-center py-8 text-gray-500 dark:text-gray-400 empty-contactos border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg">
                            <i class="fas fa-address-book text-amber-500 text-5xl mb-3"></i>
                            <p class="text-lg font-medium">No hay contactos de emergencia agregados</p>
                            <p class="text-sm mt-1">Haz clic en "Agregar Contacto" para comenzar</p>
                        </div>
                    @endforelse
                </div>
            ...
            <div class="flex justify-end gap-3 mt-6">
                <button type="button" id="guardar-salud-btn"
                    class="btn bg-success text-white border-none px-6 py-2 flex items-center gap-2 rounded-lg">
