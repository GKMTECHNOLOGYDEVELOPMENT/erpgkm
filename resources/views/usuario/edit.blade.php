<x-layout.default>
    <!-- Incluye la biblioteca Signature Pad -->
        ...
        <div class="pt-5">
            <div x-data="{ tab: 'home' }">
                ...
                <template x-if="tab === 'home'">
                    ...
                    <form id="update-form" method="POST" enctype="multipart/form-data" class="border border-[#ebedf2] dark:border-[#191e3a] rounded-md p-4 mb-5 bg-white dark:bg-[#0e1726]">
    @csrf
    @method('PUT')
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#profile-img').attr('src', 'data:image/jpeg;base64,' + btoa(String.fromCharCode.apply(null, new Uint8Array(response.usuario.avatar)))); 
                ...
                if (xhr.status === 422) {
                    ...
                    for (var field in errors) {
                        if (errors.hasOwnProperty(field)) {
                            toastr.error(errors[field].join(', ')); // Unir los mensajes de error si hay más de uno
                        }
                    }
                } else {
...
<form id="config-form" method="POST" class="border border-[#ebedf2] dark:border-[#191e3a] rounded-md p-4 bg-white dark:bg-[#0e1726]">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div>
            ...
            <select name="idSucursal" id="idSucursal" class="form-input">
                ...
                @foreach ($sucursales as $sucursal)
                    <option value="{{ $sucursal->idSucursal }}" {{ $usuario->idSucursal == $sucursal->idSucursal ? 'selected' : '' }}>{{ $sucursal->nombre }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="idTipoUsuario" id="idTipoUsuario" class="form-input">
                ...
                @foreach ($tiposUsuario as $tipoUsuario)
                    <option value="{{ $tipoUsuario->idTipoUsuario }}" {{ $usuario->idTipoUsuario == $tipoUsuario->idTipoUsuario ? 'selected' : '' }}>{{ $tipoUsuario->nombre }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="idSexo" id="idSexo" class="form-input">
                ...
                @foreach ($sexos as $sexo)
                    <option value="{{ $sexo->idSexo }}" {{ $usuario->idSexo == $sexo->idSexo ? 'selected' : '' }}>{{ $sexo->nombre }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="idRol" id="idRol" class="form-input">
                ...
                @foreach ($roles as $rol)
                    <option value="{{ $rol->idRol }}" {{ $usuario->idRol == $rol->idRol ? 'selected' : '' }}>{{ $rol->nombre }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="idTipoArea" id="idTipoArea" class="form-input">
                ...
                @foreach ($tiposArea as $tipoArea)
                    <option value="{{ $tipoArea->idTipoArea }}" {{ $usuario->idTipoArea == $tipoArea->idTipoArea ? 'selected' : '' }}>{{ $tipoArea->nombre }}</option>
                @endforeach
            </select>
        </div>
        ...
        <!-- Botones -->
        <div class="sm:col-span-2 mt-3">
            <button type="submit" class="btn btn-primary mr-2">Actualizar Información</button>
        </div>
    </div>
</form>
...
<script>
    ...
    <div class="mb-5">
        ...
        <div class="mt-3">
            <button id="clear-btn" class="btn btn-outline-success">Limpiar</button>
            <button id="save-btn" class="btn btn-outline-primary">Guardar Firma</button>
            <button id="refresh-btn" class="btn btn-outline-danger">Refrescar</button>
        </div>
...
<script>
        ...
        if (signatureCanvas && !signatureCanvas.signaturePadInstance) {
            ...
            document.getElementById('save-btn').addEventListener('click', () => {
                if (signatureCanvas.signaturePadInstance.isEmpty()) {
                    ...
                    const binaryData = dataURL.split(',')[1]; // Eliminar el encabezado de la imagen (data:image/png;base64,)

                    // Asume que tienes el ID del usuario disponible en una variable de JavaScript
                    const userId = {{ $usuario->idUsuario }};  // Asegúrate de que esta variable sea correctamente insertada desde el backend
                ...
                fetch(url)
                    ...
                    .then(data => {
                        if (data.firma) {
                            ...
                            image.src = data.firma;  // La firma es una cadena base64 con el prefijo data:image/png;base64,
                            
                            image.onload = () => {
                                // Cuando la imagen se cargue, dibujamos la firma en el lienzo
                                const ctx = signatureCanvas.getContext('2d');
                                ctx.clearRect(0, 0, signatureCanvas.width, signatureCanvas.height); // Limpiamos el lienzo
    ...
    <div class="mb-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input id="nacionalidad" name="nacionalidad" type="text" value="{{ old('nacionalidad', $usuario->nacionalidad) }}" class="form-input" />
        </div>
    ...
    <div class="mb-5">
        ...
        <div>
            ...
            <input id="direccion" name="direccion" type="text" class="form-input w-full" value="{{ old('direccion', $usuario->direccion) }}" placeholder="Ingrese la dirección">
