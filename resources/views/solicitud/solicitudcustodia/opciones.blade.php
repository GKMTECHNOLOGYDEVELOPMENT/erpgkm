<x-layout.default>
    <div class="container mx-auto px-4 py-6 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-5 bg-white rounded-xl border border-gray-200 shadow-sm">
            <div>
                ...
                <div class="flex items-center gap-2 mt-2 text-gray-600">
                    <span class="font-mono text-sm px-2.5 py-1 rounded-md bg-info-light">
            ...
            <div class="flex items-center gap-3">
                <span class="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium ring-1 ring-inset {{ $badge }}">
                    {{ $estado }}
                </span>
            </div>
        </div>
        ...
        <div class="panel rounded-xl shadow-sm overflow-hidden">
            ...
            <form method="POST" action="{{ route('solicitudcustodia.update', $custodia->id) }}" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <div class="relative">
                            <select id="estado" name="estado"
                                ...
                                @foreach (['Pendiente','En revisión','Aprobado','Rechazado','Devuelto'] as $op)
                                    <option value="{{ $op }}" @selected(old('estado', $custodia->estado) === $op)>{{ $op }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        @error('ubicacion_actual')<p class="mt-1 text-sm text-red-600 flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> {{ $message }}</p>@enderror
                    </div>
                </div>

                <div class="grid md:grid-cols-2 gap-6">
                    <!-- Fecha de devolución -->
                    <div id="wrap-fecha" class="{{ $custodia->estado === 'Devuelto' ? '' : 'opacity-70' }}">
                        <label for="fecha_devolucion" class="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4.5 w-4.5 text-indigo-500" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                            </svg>
                            Fecha de devolución
                        ...
                        <div class="relative">
                            ...
                            <input type="date" id="fecha_devolucion" name="fecha_devolucion"
                                   value="{{ old('fecha_devolucion', $custodia->fecha_devolucion) }}"
                                   class="w-full border border-gray-300 rounded-lg pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200"
                                   {{ $custodia->estado === 'Devuelto' ? '' : 'disabled' }}>
                        </div>
                        @error('fecha_devolucion')<p class="mt-1 text-sm text-red-600 flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> {{ $message }}</p>@enderror
                    </div>
                    ...
                    <div id="wrap-resp" class="{{ $custodia->estado === 'Devuelto' ? '' : 'opacity-70' }}">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="responsable_recepcion" name="responsable_recepcion" maxlength="100"
                                   value="{{ old('responsable_recepcion', $custodia->responsable_recepcion) }}"
                                   class="w-full border border-gray-300 rounded-lg pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200"
                                   placeholder="Nombre y apellido"
                                   {{ $custodia->estado === 'Devuelto' ? '' : 'disabled' }}>
                        </div>
                        @error('responsable_recepcion')<p class="mt-1 text-sm textred-600 flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> {{ $message }}</p>@enderror
                    </div>
                </div>
                ...
                <div>
                    ...
                    @error('observaciones')<p class="mt-1 text-sm text-red-600 flex items-center gap-1.5"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> {{ $message }}</p>@enderror
                </div>
