...
<div x-data="notes">
        ...
        <div class="panel p-4 flex-none w-[240px] absolute xl:relative z-10 space-y-4 h-full xl:h-auto hidden xl:block ltr:lg:rounded-r-md ltr:rounded-r-none rtl:lg:rounded-l-md rtl:rounded-l-none overflow-hidden"
            ...
            <div class="flex flex-col h-full pb-16">
                <div class="flex text-center items-center">
                    ...
                    <h3 class="text-lg font-semibold ltr:ml-3 rtl:mr-3">Notas</h3>
                </div>
                    ...
                    <div class="space-y-1">
                        <button type="button"
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ltr:ml-3 rtl:mr-3">Todas las Notas</div>
                            </div>
                        </button>
                        ...
                        <button type="button"
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ltr:ml-3 rtl:mr-3">Favoritos</div>
                            </div>
                        </button>
                        ...
                        <div class="px-1 py-3 text-white-dark flex justify-between items-center">
                            <span>Etiqueta</span>
                            <button type="button" class="text-primary hover:text-primary-dark" @click="openTagModal()"
                                title="Add New Tag">
                        ...
                        <template x-for="tag in tagsList" :key="tag.id">
                            <div class="flex items-center group">
                                <button type="button"
                                    ...
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                        xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 rotate-45 shrink-0"
                                        :class="{
                                            'fill-primary': tag.name === 'personal',
                                            'fill-warning': tag.name === 'work',
                                            'fill-info': tag.name === 'social',
                                            'fill-danger': tag.name === 'important',
                                            'fill-current': !['personal', 'work', 'social', 'important'].includes(tag
                                                .name)
                                        }"
                                        :style="!['personal', 'work', 'social', 'important'].includes(tag.name) ?
                                            `fill: ${tag.color}` : ''">
                                        <path
                                            ...
                                            <div x-data="dropdown" @click.outside="open = false"
                                                ...
                                                <a href="javascript:;"
                                                    ...
                                                    <svg width="24" height="24" viewBox="0 0 24 24"
                                                        ...
                                                        :style="!['personal', 'work', 'social', 'important'].includes(note
                                                            .tag) ? `fill: ${note.tag_color}` : ''">
                                                        <path
                                                            d="M2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2C16.714 2 19.0711 2 20.5355 3.46447C22 4.92893 22 7.28595 22 12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12Z"
                                                            stroke="currentColor" stroke-width="1.5"></path>
                                                    </svg>
                                                ...
                                                <ul x-cloak x-show="open" x-transition x-transition.duration.300ms
                                                    ...
                                                    <template x-for="tag in tagsList" :key="tag.id">
                                                        <li>
                                                            <a href="javascript:;"
                                                                @click="toggle, setTag(note, tag.id)">
                                                                <svg width="24" height="24"
                                                                    viewBox="0 0 24 24" fill="none"
                                                                    xmlns="http://www.w3.org/2000/svg"
                                                                    class="w-3 h-3 rotate-45 ltr:mr-2 rtl:ml-2"
                                                                    :class="{
                                                                        'fill-primary': tag.name === 'personal',
                                                                        'fill-warning': tag.name === 'work',
                                                                        'fill-info': tag.name === 'social',
                                                                    ...
                                                                    :style="!['personal', 'work', 'social', 'important']
                                                                    .includes(tag.name) ? `fill: ${tag.color}` : ''">
                                                                    <path
                                                                        d="M2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2C16.714 2 19.0711 2 20.5355 3.46447C22 4.92893 22 7.28595 22 12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12Z"
                ...
                <div class="p-5">
                    <form @submit.prevent="saveNote">
                        <div class="mb-5">
                            ...
                            <input id="title" type="text" placeholder="Ingrese el Titulo" class="form-input"
                            ...
                            <select id="tag_id" class="form-select" x-model="params.tag_id">
                                ...
                                <template x-for="tag in tagsList" :key="tag.id">
                                    <option :value="tag.id" x-text="tag.name"
                                        :selected="params.tag_id === tag.id"></option>
                                </template>
                            </select>
                        </div>
                        <div class="mb-5">
                            <label for="description">Descripcion</label>
                            <textarea id="description" rows="3" class="form-textarea resize-none min-h-[130px]"
                                placeholder="Ingrese la Descripcion" x-model="params.description"></textarea>
                        </div>
                        <div class="mb-5">
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="checkbox" class="form-checkbox" x-model="params.is_favorite">
                                <span class="ltr:ml-2 rtl:mr-2">Marcar como Favorito</span>
                            </label>
                        </div>
                        <div class="flex justify-end items-center mt-8">
                            <button type="button" class="btn btn-outline-danger gap-2"
                                @click="isAddNoteModal = false">Cancelar</button>
                            <button type="submit" class="btn btn-primary ltr:ml-4 rtl:mr-4"
                                x-text="params.id ? 'Actualizar Nota' : 'Agregar Nota'"></button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="flex items-center justify-center min-h-screen">
            <div x-show="isDeleteNoteModal" x-transition x-transition.duration.300
                ...
                <div class="p-5 text-center">
                    ...
                    <div class="sm:w-3/4 mx-auto mt-5">¿Estás seguro de que deseas eliminar esta nota?</div>
                    <div class="flex justify-center items-center mt-8">
                        <button type="button" class="btn btn-outline-danger"
                            @click="isDeleteNoteModal = false">Cancelar</button>
                        <button type="button" class="btn btn-primary ltr:ml-4 rtl:mr-4"
                            @click="deleteNote">Eliminar</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div x-show="isViewNoteModal" x-transition x-transition.duration.300
                ...
                <div class="p-5">
                    ...
                    <div class="ltr:text-right rtl:text-left mt-8">
                        <button type="button" class="btn btn-outline-danger"
                            @click="isViewNoteModal = false">Cerrar</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div x-show="isTagModal" x-transition x-transition.duration.300 @click.outside="isTagModal = false"
                ...
                <div class="p-5">
                    <form @submit.prevent="saveTag">
                        <div class="mb-5">
                            <label for="tag_name">Nombre de la Etiqueta</label>
                            <input id="tag_name" type="text" placeholder="Ingrese el Nombre de la Etiqueta" class="form-input"
                        ...
                        <div class="mb-5">
                            <label for="tag_description">Descripción (Opcional)</label>
                            <textarea id="tag_description" rows="2" class="form-textarea" placeholder="Ingrese la Descripción"
                                x-model="tagParams.description"></textarea>
                        </div>
                        <div class="flex justify-end items-center mt-8">
                            <button type="button" class="btn btn-outline-danger gap-2"
                                @click="isTagModal = false">Cancelar</button>
                            <button type="submit" class="btn btn-primary ltr:ml-4 rtl:mr-4"
                                x-text="tagParams.id ? 'Actualizar Etiqueta' : 'Agregar Etiqueta'"></button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <!-- Delete Tag Modal -->
    <div class="fixed inset-0 bg-[black]/60 z-[999] px-4 overflow-y-auto hidden"
        :class="isDeleteTagModal && '!block'">
        <div class="flex items-center justify-center min-h-screen">
            <div x-show="isDeleteTagModal" x-transition x-transition.duration.300
                @click.outside="isDeleteTagModal = false"
                class="panel border-0 p-0 rounded-lg overflow-hidden md:w-full max-w-lg w-[90%] my-8">
                <button type="button" class="absolute top-4 ltr:right-4 rtl:left-4 text-white-dark hover:text-dark"
                ...
                <div class="p-5 text-center">
                    ...
                    <div class="sm:w-3/4 mx-auto mt-5">
                        ¿Estás seguro de que deseas eliminar la etiqueta "<span x-text="tagToDelete?.name"
                            class="font-semibold"></span>"?
                        <div x-show="tagToDelete?.notes_count > 0" class="mt-2 text-danger">
                            Esta etiqueta se utiliza en <span x-text="tagToDelete?.notes_count"></span> notas.
                        </div>
                    </div>
                    <div class="flex justify-center items-center mt-8">
                        <button type="button" class="btn btn-outline-danger"
                            @click="isDeleteTagModal = false">Cancelar</button>
                        <button type="button" class="btn btn-primary ltr:ml-4 rtl:mr-4"
                            @click="deleteTag">Eliminar</button>
                    </div>
                </div>
            </div>
        </div>
