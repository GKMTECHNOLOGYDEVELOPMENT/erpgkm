...
<div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#121c2c]">
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white"><i class="fas fa-list mr-2"></i>Lista de
                Repuestos</h3>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-[#121c2c] transition-colors">
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <div class="text-sm font-semibold text-gray-900 dark:text-white">
                                        {{ $item->nombre_repuesto }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-500 mt-1 flex flex-wrap gap-1">
                                        @if ($item->subcategoria)
                                            <span
                                                class="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 rounded-full">{{ $item->subcategoria }}</span>
                                        @endif
                                        @if ($item->modelo)
                                            <span
                                                class="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-full">{{ $item->modelo }}</span>
                                        @endif
                                        @if ($item->marca)
                                            <span
                                                class="px-2 py-0.5 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-full">{{ $item->marca }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                                switch ($item->estado_entrega) {
                                    ...
                                    case 'cedido':
                                        $estadoClase =
                                            'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-400'; // Cambiado a purple
                                        $estadoIcono = '<i class="fas fa-exchange-alt mr-1"></i>';
                                        $estadoTexto = 'Cedido';
                                        break;
                                    case 'devuelto':
                                    ...
                                    case 'pendiente_entrega':
                                        ...
                                        break;
                                    case 'pendiente_por_retorno':
                                        $estadoClase = 'bg-warning-light text-warning border border-warning';
                                        $estadoIcono = '<i class="fas fa-undo mr-1"></i>';
                                        $estadoTexto = 'Pendiente por Retorno';
                                        break;
                            ...
                            <div class="flex justify-center">
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {!! $estadoClase !!}">{!! $estadoIcono !!}
                                    {{ $estadoTexto }}</span>
                            </div>
                            @if ($item->numero_ticket)
                                <div class="text-xs text-gray-500 dark:text-gray-500 mt-1 text-center"><i
                                        class="fas fa-ticket-alt mr-1"></i>{{ $item->numero_ticket }}</div>
                            @endif
                        </td>
                            ...
                            <div class="space-y-1">
                                @if ($item->fechaUsado)
                                    <div class="flex items-center justify-center">
                                        ...
                                        <span
                                            class="ml-1">{{ \Carbon\Carbon::parse($item->fechaUsado)->format('d/m/Y H:i') }}</span>
                                    </div>
                                @endif
                                @if ($item->fechaSinUsar)
                                    <div class="flex items-center justify-center">
                                        ...
                                        <span
                                            class="ml-1">{{ \Carbon\Carbon::parse($item->fechaSinUsar)->format('d/m/Y H:i') }}</span>
                                    </div>
                                @endif
                                @if (!$item->fechaUsado && !$item->fechaSinUsar)
                                    <div class="flex items-center justify-center">
                                        ...
                                        <span
                                            class="ml-1">{{ \Carbon\Carbon::parse($item->fecha_solicitud)->format('d/m/Y H:i') }}</span>
                                    </div>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            @php
                                ...
                                $dias =
                                    $inicio && $fin ? (int) $inicio->startOfDay()->diffInDays($fin->startOfDay()) : 0;
                            @endphp
                            <span
                                class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold {{ $dias >= 10 ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : ($dias >= 5 ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400') }}">
                                {{ $dias }} días
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-center">
                                <span
                                    class="inline-flex items-center justify-center px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400 text-sm font-semibold">{{ $item->cantidad }}
                                    unidades</span>
                            </div>
                            @if ($item->observacion)
                                <div class="text-xs text-gray-500 dark:text-gray-500 mt-2 truncate max-w-xs text-center"
                                    title="{{ $item->observacion }}"><i
                                        class="fas fa-sticky-note mr-1"></i>{{ Str::limit($item->observacion, 50) }}
                                </div>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center justify-center space-x-2">
                                <button type="button" onclick="openDetailsModal({{ $item->idOrdenesArticulos }})"
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center">
                        <div class="text-gray-400 dark:text-gray-500">
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">No se encontraron repuestos con los
                                filtros aplicados.</p>
                        </div>
                    </td>
                </tr>
    ...
    <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#121c2c]">
        <div class="flex items-center justify-between">
            <div class="text-sm text-gray-700 dark:text-gray-400">
                Mostrando {{ $repuestos->firstItem() }} a {{ $repuestos->lastItem() }} de {{ $repuestos->total() }}
                resultados
            </div>
