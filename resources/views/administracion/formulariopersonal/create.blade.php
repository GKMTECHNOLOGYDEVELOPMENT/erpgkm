...
<body class="min-h-screen p-4 md:p-6">
    ...
    <div class="w-full mx-auto">
        ...
        <!-- Formulario -->
        <form id="personal-data-form" class="space-y-8" method="POST"
            action="{{ route('admin.formulario-personal.store') }}" enctype="multipart/form-data">
            @csrf

            {{-- ✅ NUEVO: para marcar uso único en store() --}}
            <input type="hidden" name="form_link_id" value="{{ $formLinkId ?? '' }}">

            {{-- ✅ OPCIONAL: si también quieres enviar el token --}}
            <input type="hidden" name="form_link_token" value="{{ request('token') }}">
    ...
    const FormDataCollector = {
        ...
        collectAllData: function() {
            ...
            niveles.forEach(nivel => {
                const terminoDesktop = document.querySelector(
                    `[name="${nivel}_termino"]:not([id$="_mobile"]):checked`);
                const terminoMobile = document.querySelector(
                    `[name="${nivel}_termino"][id$="_mobile"]:checked`);
                ...
                if (termino) {
                    const estudio = {
                        ...
                        especialidad: document.querySelector(`[name="${nivel}_especialidad"]`)
                            ?.value || '',
            ...
            document.querySelectorAll('[data-familiar-id]').forEach(familiarRow => {
                ...
                const nombres = document.querySelector(`[name="familiares[${familiarId}][nombres]"]`)
                    ?.value;
                if (nombres) {
                    familiares.push({
                        parentesco: document.querySelector(
                            `[name="familiares[${familiarId}][parentesco]"]`)?.value,
                        nombres: nombres,
                        documento: document.querySelector(
                            `[name="familiares[${familiarId}][documento]"]`)?.value,
                        ocupacion: document.querySelector(
                            `[name="familiares[${familiarId}][ocupacion]"]`)?.value,
                        sexo: document.querySelector(`[name="familiares[${familiarId}][sexo]"]`)
                            ?.value,
                        fecha_nacimiento: document.querySelector(
                            `[name="familiares[${familiarId}][fecha_nacimiento]"]`)?.value,
                        domicilio: document.querySelector(
                            `[name="familiares[${familiarId}][domicilio]"]`)?.value
                    });
                }
            });
            ...
            for (let i = 3; i <= 5; i++) {
                ...
                if (nombres) {
                    ...
                    data[`emergencia${i}_parentesco`] = document.querySelector(
                        `[name="emergencia${i}_parentesco"]`)?.value;
                    data[`emergencia${i}_direccion`] = document.querySelector(
                        `[name="emergencia${i}_direccion"]`)?.value;
                }
            }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function calculateProgress() {
            ...
            const percentage = requiredFields.length > 0 ? Math.round((completed / requiredFields.length) *
                100) : 0;
        ...
        document.getElementById('save-draft')?.addEventListener('click', function(e) {
            ...
            const {
                data,
                formData
            } = FormDataCollector.collectAllData();
        ...
        document.getElementById('personal-data-form')?.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    submitBtn.innerHTML =
                        '<i class="fas fa-spinner fa-spin mr-2"></i><span>Enviando...</span>';
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            ...
                            submitBtn.innerHTML =
                                '<i class="fas fa-paper-plane mr-2"></i><span>Enviar Formulario Completado</span>';
